...
const pkcs12File = () =>
  new File([PKCS12_ARCHIVE().buffer as ArrayBuffer], "server.p12", {
...
const setup = () => {
  ...
  m("GetKeep").mockResolvedValue({
    UseKeepConnect_bool: false,
    KeepConnectHost_str: "keepalive.softether.org",
    KeepConnectPort_u32: 80,
    KeepConnectProtocol_u32: 0,
    KeepConnectInterval_u32: 50,
  });
  m("GetSysLog").mockResolvedValue({ SaveType_u32: 0, Hostname_str: "", Port_u32: 514 });
  m("GetSpecialListener").mockResolvedValue({
...
describe("EncryptionNetwork", () => {
  ...
  it("decrypts and imports a PKCS #12 certificate in the browser", async () => {
    ...
    await user.upload(fileInput, pkcs12File());
    await user.type(within(c).getByLabelText("PKCS #12 archive password"), PKCS12_PASSWORD);
    ...
    expect(JSON.stringify(sent)).not.toContain(PKCS12_PASSWORD);
