  ...
  const change = () => {
    api
      ...
      .then(() => {
        ...
        setStatus({
          variant: "success",
          text: "The server administrator password has been changed.",
        });
      })
...
const ServerCertCard: React.FunctionComponent = () => {
  ...
  const importCert = async () => {
    ...
    try {
      ...
      await api.SetServerCert(
        new VPN.VpnRpcKeyPair({
  ...
  return (
    <Card>
            ...
            <StackItem>
              ...
              <Form>
                    ...
                    onFileInputChange={(_e, file) => {
                      ...
                      readFileBytes(file)
                        ...
                        .catch(() =>
                          setError(
                            archive
                              ? "Could not read the PKCS #12 archive."
                    ...
                    <FormGroup label="Private key" fieldId="key-file">
                      <FileUpload
                        ...
                        onFileInputChange={(_e, file) => {
                          ...
                          readFileBytes(file)
                            .then((b) => (keyBytes.current = b))
                            .catch(() => setError("Could not read the private key file."));
                        }}
                        onClearClick={() => {
                          setKeyFile("");
                          keyBytes.current = null;
                  ...
                  <>
                    <FormGroup label="Archive password" fieldId="pkcs12-password">
                      ...
                      <FormHelperText>
                        <HelperText>
                          <HelperTextItem>
                            The password is used only in this browser to decrypt the archive.
                          </HelperTextItem>
                        </HelperText>
                      </FormHelperText>
                    </FormGroup>
                ...
                <Button
                  ...
                  isDisabled={
                    importing ||
                    (certificateIsPkcs12 ? pkcs12Bytes === null : certFile === "" || keyFile === "")
                  }
                  isLoading={importing}
                >
                  Import certificate
      ...
      >
        ...
        <ModalBody>
          <Content component="p">
            A new self-signed certificate is generated with the common name (CN) you specify. Set
            the CN to the hostname clients use to reach this server (important for SSTP).
          </Content>
          <Form>
...
const CipherCard: React.FunctionComponent = () => {
  ...
  const save = () => {
    ...
    setSaving(true);
    api
      .SetServerCipher(new VPN.VpnRpcStr({ String_str: cipher }))
    ...
    <Card>
      ...
      <CardBody>
        {error && (
          <Alert
            variant="danger"
            title="Could not load or save the cipher"
            isInline
            style={{ marginBlockEnd: "var(--pf-t--global--spacer--md)" }}
          >
            {error}
        ...
        {cipher === null ? (
          <Bullseye>
            <Spinner size="lg" aria-label="Loading cipher" />
          </Bullseye>
        ) : (
          <Form>
            ...
            <Button variant="primary" onClick={save} isDisabled={saving} isLoading={saving}>
              Save
            </Button>
          </Form>
        )}
...
const KeepAliveCard: React.FunctionComponent = () => {
  ...
  const load = React.useCallback(() => {
    setError(null);
    api
      .GetKeep(new VPN.VpnRpcKeep())
      .then((r) => setKeep(r as unknown as Record<string, unknown>))
      .catch((e) => setError(String(e)));
  }, []);
  React.useEffect(() => {
    load();
  ...
  const setField = (k: string, v: unknown) =>
    setKeep((prev) => (prev ? { ...prev, [k]: v } : prev));
  const save = () => {
    if (!keep) return;
    setSaving(true);
    api
      .SetKeep(new VPN.VpnRpcKeep(keep as Partial<VPN.VpnRpcKeep>))
      .then(() => {
        setSaving(false);
        load();
      })
    ...
    <Card>
      ...
      <CardBody>
        {error && (
          <Alert
            variant="danger"
            title="Could not load or save keep-alive"
            isInline
            style={{ marginBlockEnd: "var(--pf-t--global--spacer--md)" }}
          >
            {error}
        ...
        {keep === null ? (
          <Bullseye>
            <Spinner size="lg" aria-label="Loading keep-alive" />
          </Bullseye>
        ) : (
          <Form>
            <Switch
              id="keep"
              label="Send keep-alive packets to keep the internet connection alive"
              isChecked={enabled}
              onChange={(_e, c) => setField("UseKeepConnect_bool", c)}
            />
            <FormGroup label="Host name" fieldId="keep-host">
              <TextInput
                id="keep-host"
                value={String(keep.KeepConnectHost_str ?? "")}
                onChange={(_e, v) => setField("KeepConnectHost_str", v)}
                isDisabled={!enabled}
                aria-label="Keep-alive host name"
              />
            </FormGroup>
            <FormGroup label="Port" fieldId="keep-port">
              <TextInput
                ...
                min={1}
                max={65535}
                value={String(keep.KeepConnectPort_u32 ?? 0)}
                onChange={(_e, v) => setField("KeepConnectPort_u32", Number(v) || 0)}
                isDisabled={!enabled}
                aria-label="Keep-alive port"
              />
            </FormGroup>
            <FormGroup label="Protocol" fieldId="keep-proto">
              <FormSelect
                id="keep-proto"
                value={String(keep.KeepConnectProtocol_u32 ?? 0)}
                onChange={(_e, v) => setField("KeepConnectProtocol_u32", Number(v))}
                isDisabled={!enabled}
                aria-label="Keep-alive protocol"
              >
            ...
            <FormGroup label="Interval (seconds)" fieldId="keep-interval">
              <TextInput
                ...
                id="keep-interval"
                min={1}
                value={String(keep.KeepConnectInterval_u32 ?? 0)}
                onChange={(_e, v) => setField("KeepConnectInterval_u32", Number(v) || 0)}
                isDisabled={!enabled}
...
const SyslogCard: React.FunctionComponent = () => {
  ...
  const setField = (k: string, v: unknown) =>
    setSyslog((prev) => (prev ? { ...prev, [k]: v } : prev));
  ...
  const save = () => {
    ...
    setSaving(true);
    api
      .SetSysLog(new VPN.VpnSyslogSetting(syslog as Partial<VPN.VpnSyslogSetting>))
      .then(() => {
        setSaving(false);
        load();
      })
    ...
    <Card>
      ...
      <CardBody>
        {error && (
          <Alert
            variant="danger"
            title="Could not load or save syslog settings"
            isInline
            style={{ marginBlockEnd: "var(--pf-t--global--spacer--md)" }}
          >
            {error}
        ...
        {syslog === null ? (
          <Bullseye>
            <Spinner size="lg" aria-label="Loading syslog" />
          </Bullseye>
        ) : (
          <Form>
            <FormGroup label="Send to a syslog server" fieldId="syslog-type">
              <FormSelect
                id="syslog-type"
                value={String(syslog.SaveType_u32 ?? 0)}
                onChange={(_e, v) => setField("SaveType_u32", Number(v))}
                aria-label="Syslog save type"
              >
            ...
            <FormGroup label="Syslog host name" fieldId="syslog-host">
              <TextInput
                id="syslog-host"
                value={host}
                onChange={(_e, v) => setField("Hostname_str", v)}
                isDisabled={!enabled}
                validated={hostValid ? "default" : "error"}
                aria-label="Syslog host name"
              />
              {!hostValid && (
                <FormHelperText>
                  <HelperText>
                    <HelperTextItem variant="error">
                      Enter the syslog server host name.
                    </HelperTextItem>
                  </HelperText>
                </FormHelperText>
              )}
            ...
            <FormGroup label="Port" fieldId="syslog-port">
              <TextInput
                ...
                min={1}
                max={65535}
                value={String(syslog.Port_u32 ?? 514)}
                onChange={(_e, v) => setField("Port_u32", Number(v) || 0)}
                isDisabled={!enabled}
                validated={portValid ? "default" : "error"}
                aria-label="Syslog port"
              />
              {!portValid && (
                <FormHelperText>
                  <HelperText>
...
const SpecialListenerCard: React.FunctionComponent = () => {
  ...
  const load = React.useCallback(() => {
    ...
    api
      .GetSpecialListener()
      .then((r) => setListener(r as unknown as Record<string, unknown>))
      .catch((e) => setError(String(e)));
  }, []);
  React.useEffect(() => {
    load();
  ...
  const setField = (k: string, v: unknown) =>
    setListener((prev) => (prev ? { ...prev, [k]: v } : prev));
  const save = () => {
    if (!listener) return;
    setSaving(true);
    api
      .SetSpecialListener(
        new VPN.VpnRpcSpecialListener(listener as Partial<VPN.VpnRpcSpecialListener>),
      )
      .then(() => {
        setSaving(false);
        load();
    ...
    <Card>
      ...
      <CardBody>
        {error && (
          <Alert
            variant="danger"
            title="Could not load or save VPN over ICMP/DNS"
            isInline
            style={{ marginBlockEnd: "var(--pf-t--global--spacer--md)" }}
          >
            {error}
        ...
        {listener === null ? (
          <Bullseye>
            <Spinner size="lg" aria-label="Loading VPN over ICMP/DNS" />
          </Bullseye>
        ) : (
          <Form>
            <Switch
              id="vpn-icmp"
              label="Enable VPN over ICMP"
              isChecked={listener.VpnOverIcmpListener_bool === true}
              onChange={(_e, c) => setField("VpnOverIcmpListener_bool", c)}
            />
            <Switch
              id="vpn-dns"
              label="Enable VPN over DNS (UDP port 53)"
              isChecked={listener.VpnOverDnsListener_bool === true}
              onChange={(_e, c) => setField("VpnOverDnsListener_bool", c)}
            />
            <Button variant="primary" onClick={save} isDisabled={saving} isLoading={saving}>
              Save
            </Button>
