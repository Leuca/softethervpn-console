...
describe("ClusterConfig", () => {
  ...
  it("seeds the member fields from the response", async () => {
    ...
    expect(
      (screen.getByLabelText("Controller host name or IP address") as HTMLInputElement).value,
    ).toBe("ctrl.example.com");
  });
  ...
  it("shows a restarting notice (not an error) after saving", async () => {
    ...
    expect(
      screen.queryByText("Could not load or save the clustering configuration"),
