...
describe("DynDNS", () => {
  ...
  it("offers to regenerate the server cert when its CN no longer matches the FQDN", async () => {
    ...
    getStatus.mockResolvedValueOnce(
      status({ CurrentFqdn_str: CERT_FQDN, CurrentHostName_str: CERT_FQDN.split(".")[0] }),
    );
    getStatus.mockResolvedValue(
      status({ CurrentFqdn_str: "newname.softether.net", CurrentHostName_str: "newname" }),
    );
    changeHostname.mockResolvedValue({});
  ...
  it("does not prompt to regenerate when the cert CN already matches", async () => {
    ...
    getStatus.mockResolvedValue(
      status({ CurrentFqdn_str: CERT_FQDN, CurrentHostName_str: "oldname" }),
    );
    changeHostname.mockResolvedValue({});
  ...
  it("degrades to an info note when the proxy RPC is unsupported (code 33)", async () => {
    ...
    expect(
      await screen.findByText("Proxy configuration is not supported by this server"),
    ).toBeInTheDocument();
