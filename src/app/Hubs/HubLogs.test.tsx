...
vi.mock("@app/utils/blob_utils", async () => {
  const actual =
    await vi.importActual<typeof import("@app/utils/blob_utils")>("@app/utils/blob_utils");
...
describe("HubLogs", () => {
  ...
  it("lists log files and previews a file downloaded in chunks", async () => {
    ...
    const row = screen
      .getByText("@security_log/DEFAULT/20260706.log")
  ...
  it("downloads a log file", async () => {
    ...
    enumLogFile.mockResolvedValue({ LogFiles: [logFile] });
    readLogFile
      .mockResolvedValueOnce({ Buffer_bin: b64("log body") })
      .mockResolvedValueOnce({ Buffer_bin: "" });
    const user = userEvent.setup();

    render(<HubLogs hub="DEFAULT" />);
    await user.click(screen.getByRole("tab", { name: "Files" }));
    const row = (await screen.findByText("@security_log/DEFAULT/20260706.log")).closest(
      "tr",
  ...
  it("shows capability messages when log RPCs are unsupported", async () => {
    ...
    expect(
      await screen.findByText("Log settings are not supported by this server"),
    ).toBeInTheDocument();
    expect(getHubLog).not.toHaveBeenCalled();

    await user.click(screen.getByRole("tab", { name: "Files" }));
    expect(
      await screen.findByText("Log file browsing is not supported by this server"),
    ).toBeInTheDocument();
