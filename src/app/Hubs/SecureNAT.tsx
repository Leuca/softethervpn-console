...
const isValidOptionalIpv4 = (value: string): boolean =>
  value === "" || value === "0.0.0.0" || isValidIpv4(value);
...
const numberField = (config: Record<string, unknown>, key: string): number =>
  Number(config[key] ?? 0);

const fieldText = (config: Record<string, unknown>, key: string): string =>
  String(config[key] ?? "");
...
const SecureNatRuntimeModal: React.FunctionComponent<{
  hub: string;
  isOpen: boolean;
  onClose: () => void;
}> = ({ hub, isOpen, onClose }) => {
  ...
  return (
    <Modal
      ...
      isOpen={isOpen}
      onClose={onClose}
      aria-label="Secure NAT runtime information"
    >
        ...
        <Flex direction={{ default: "column" }} gap={{ default: "gapMd" }}>
          ...
          {isInitialLoading ? (
            ...
            <>
              ...
              <KeyValueTable
                data={runtime.status as unknown as Record<string, unknown>}
                ariaLabel="Secure NAT status"
              />
                    ...
                    {runtime.nat.map((item) => (
                      <Tr key={item.Id_u32}>
                        ...
                        <Td dataLabel="Protocol">
                          {protocolLabels[item.Protocol_u32] ?? `Protocol ${item.Protocol_u32}`}
                        </Td>
                        <Td dataLabel="Source">{`${item.SrcIp_ip}:${item.SrcPort_u32}`}</Td>
                        <Td dataLabel="Destination">{`${item.DestIp_ip}:${item.DestPort_u32}`}</Td>
                        <Td dataLabel="Last communication">
                          {formatRpcValue("LastCommTime_dt", item.LastCommTime_dt)}
                        </Td>
              ...
              {runtime.dhcp.length === 0 ? (
                <EmptyState titleText="No DHCP leases" headingLevel="h3">
                  <EmptyStateBody>
                    No DHCP clients currently have leases from Secure NAT.
                  </EmptyStateBody>
                </EmptyState>
              ) : (
                  ...
                  <Tbody>
                    {runtime.dhcp.map((item) => (
                      <Tr key={item.Id_u32}>
                        ...
                        <Td dataLabel="Leased">
                          {formatRpcValue("LeasedTime_dt", item.LeasedTime_dt)}
                        </Td>
                        <Td dataLabel="Expires">
                          {formatRpcValue("ExpireTime_dt", item.ExpireTime_dt)}
                        </Td>
                      </Tr>
                    ))}
                  </Tbody>
  ...
  const load = React.useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([hubStatus, secureNatOption]) => {
        ...
        const option = normalizeSecureNatOption(
          secureNatOption as unknown as Record<string, unknown>,
        );
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        <>
          <Alert
            ...
            title={enabled ? "Secure NAT is enabled" : "Secure NAT is disabled"}
            isInline
          />
          ...
          <Form>
              ...
              <GridItem span={12} md={6}>
                <FormGroup label="Subnet mask" fieldId="securenat-mask">
                  <TextInput
                    ...
                    validated={
                      isValidSubnetMask(fieldText(config, "Mask_ip")) ? "default" : "error"
                    }
            ...
            {virtualNatDisabled && (
              <Alert variant="info" title="This server does not support Virtual NAT" isInline />
            )}
            <Grid hasGutter>
              <GridItem span={12}>
                ...
                <FormGroup label="Lease time (seconds)" fieldId="securenat-dhcp-expire">
                  <TextInput
                    ...
                    onChange={(_event, value) =>
                      setField("DhcpExpireTimeSpan_u32", Number(value) || 0)
                    }
                ...
                <GridItem span={12}>
                  <FormGroup label="Static routes to push" fieldId="securenat-dhcp-routes">
                    ...
                    <FormHelperText>
                      <HelperText>
                        <HelperTextItem>
                          Use network/subnet/gateway entries separated by commas or spaces.
                        </HelperTextItem>
                      </HelperText>
                    </FormHelperText>
                  </FormGroup>
          ...
          {enabled && (
            <section>
              ...
              <Content component="p">
                Inspect the current Secure NAT operating status, NAT sessions, and DHCP leases. The
                runtime view auto-refreshes while open.
              </Content>
      ...
      >
        ...
        <ModalBody>
          Secure NAT starts a virtual NAT router and DHCP server inside this Virtual Hub. Enable it
          only when the network settings are intentional.
        </ModalBody>
        <ModalFooter>
          <Button
            variant="primary"
