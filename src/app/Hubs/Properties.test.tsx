...
describe("Properties", () => {
      ...
      AdminOptionList: [
        {
          Name_str: "NoIpTable",
          Value_u32: 0,
          Descrption_utf: "Do not generate an IP address table.",
        },
        {
  ...
  it("loads hub administration options", async () => {
    ...
    const adminDialog = await screen.findByRole("dialog", {
      name: "Virtual Hub Administration Options",
    });

    expect(
      await within(adminDialog).findByText("allow_hub_admin_change_option"),
    ).toBeInTheDocument();
    expect(
      within(adminDialog).getByText("Allow hub admins to change admin options."),
    ).toBeInTheDocument();
  ...
  it("shows an error when hub administration options fail to load", async () => {
    ...
    expect(
      await screen.findByText("Could not load or save hub administration options"),
    ).toBeInTheDocument();
  });
  ...
  it("edits hub administration options inline before saving", async () => {
    ...
    expect(
      within(adminDialog).queryByRole("button", { name: "Add option" }),
    ).not.toBeInTheDocument();
    expect(
      within(adminDialog).queryByRole("button", { name: /kebab toggle/i }),
    ).not.toBeInTheDocument();

    await user.click(
      await within(adminDialog).findByLabelText("Value for allow_hub_admin_change_option"),
    );
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole("dialog", { name: "Virtual Hub Administration Options" }),
      ).not.toBeInTheDocument(),
    );
  });
  ...
  it("shows an error when hub extended options fail to load", async () => {
    ...
    expect(
      await screen.findByText("Could not load or save hub extended options"),
    ).toBeInTheDocument();
  });
  ...
  it("edits hub extended options inline before saving", async () => {
    ...
    const threshold = within(extDialog).getByLabelText(
      "Value for BroadcastStormDetectionThreshold",
    );
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole("dialog", { name: "Virtual Hub Extended Options" }),
      ).not.toBeInTheDocument(),
    );
  });
  ...
  it("shows an error when source IP access control fails to load", async () => {
    ...
    expect(
      await screen.findByText("Could not load or save source IP access control"),
    ).toBeInTheDocument();
  });
  ...
  it("edits and deletes source IP access control rules before saving", async () => {
    ...
    const afterDeleteDialog = await screen.findByRole("dialog", {
      name: "Source IP Access Control",
    });
    expect(
      within(afterDeleteDialog).queryByText("192.0.2.10/255.255.255.0"),
    ).not.toBeInTheDocument();
