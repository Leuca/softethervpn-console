...
describe("Cascade", () => {
  ...
  it("offers all four client-auth methods including RADIUS/NT and certificate", async () => {
    ...
    expect(
      within(authSelect).getByRole("option", { name: "Standard password" }),
    ).toBeInTheDocument();
    expect(
      within(authSelect).getByRole("option", { name: /RADIUS \/ NT domain/ }),
    ).toBeInTheDocument();
    expect(
      within(authSelect).getByRole("option", { name: "Client certificate" }),
    ).toBeInTheDocument();
  });
  ...
  it("hashes the password for standard (SHA-0) auth", async () => {
    ...
    expect(Array.from(sent.HashedPassword_bin)).toEqual(
      Array.from(hashSoftEtherPassword("alice", "secret")),
    );
  });
  ...
  it("rejects an encrypted private key with a clear error", async () => {
    ...
    expect(
      await within(dialog).findByText(/Encrypted private-key files cannot be imported separately/i),
    ).toBeInTheDocument();
  ...
  it("rehashes the password when editing standard-password auth", async () => {
    ...
    const sent = setLink.mock.calls[0][0];
    expect(Array.from(sent.HashedPassword_bin)).toEqual(
      Array.from(hashSoftEtherPassword("alice", "newsecret")),
    );
  });
  ...
  it("blocks half-duplex when only one TCP connection is selected", async () => {
    ...
    await user.click(
      within(dialog).getByLabelText("Use half-duplex mode (with multiple connections)"),
    );
  ...
  it("edits the security policy of a cascade", async () => {
    ...
    expect(
      screen.queryByRole("switch", { name: /apply a security policy/i }),
  ...
  it("sets an online cascade offline", async () => {
    ...
    expect(setLinkOffline.mock.calls[0][0]).toMatchObject({
      HubName_str: "DEFAULT",
      AccountName_utf: "to-branch",
    });
  });
  ...
  it("sets an offline cascade online", async () => {
    ...
    expect(setLinkOnline.mock.calls[0][0]).toMatchObject({
      HubName_str: "DEFAULT",
      AccountName_utf: "to-branch",
    });
  });
  ...
  it("inspects and edits an existing cascade, preserving auth on save", async () => {
    ...
    expect(getLink.mock.calls[0][0]).toMatchObject({
      HubName_Ex_str: "DEFAULT",
      AccountName_utf: "to-branch",
    });
    const sent = setLink.mock.calls[0][0];
  ...
  it("opens the connection status modal", async () => {
    ...
    getLinkStatus.mockResolvedValue({
      Connected_bool: true,
      ServerName_str: "branch.example.com",
      ServerPort_u32: 443,
    });
    const user = userEvent.setup();
    ...
    expect(getLinkStatus.mock.calls[0][0]).toMatchObject({
      HubName_Ex_str: "DEFAULT",
      AccountName_utf: "to-branch",
    });
  });
  ...
  it("deletes a cascade after confirmation", async () => {
    ...
    expect(deleteLink.mock.calls[0][0]).toMatchObject({
      HubName_str: "DEFAULT",
      AccountName_utf: "to-branch",
    });
  });
