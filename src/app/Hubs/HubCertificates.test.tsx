...
describe("HubCertificates", () => {
  ...
  it("adds a trusted CA certificate and normalizes PEM uploads to DER", async () => {
    ...
    await user.upload(
      fileInput,
      new File([SELF_SIGNED_CERT_PEM()], "root.pem", { type: "application/x-pem-file" }),
    );
  ...
  it("lists certificate revocation entries for the hub", async () => {
    enumCa.mockResolvedValue({ CAList: [] });
    enumCrl.mockResolvedValue({
      CRLList: [{ Key_u32: 9, CrlInfo_utf: "CN=revoked.example.com, Serial=01" }],
    });
  ...
  it("adds a certificate revocation entry from a certificate upload", async () => {
    ...
    await user.upload(
      ...
      new File([SELF_SIGNED_CERT_PEM()], "revoked.pem", { type: "application/x-pem-file" }),
    );

    await waitFor(() =>
      expect(within(dialog).getByLabelText("Common name")).toHaveValue("test.example.com"),
    );
  ...
  it("edits and deletes a certificate revocation entry", async () => {
    ...
    expect(within(editDialog).getByLabelText("SHA1 digest")).toHaveValue(
