...
describe("Users", () => {
  ...
  it("creates an anonymous user", async () => {
    ...
    expect(
      new Date(createUser.mock.calls[0][0].ExpireTime_dt).getUTCFullYear(),
    ).toBeLessThanOrEqual(1970);
  });
  ...
  it("creates a user authenticated by a registered certificate", async () => {
    ...
    await user.upload(
      fileInput,
      new File([SELF_SIGNED_CERT_DER()], "user.cer", { type: "application/x-x509-ca-cert" }),
    );
    expect(
      await within(dialog).findByRole("button", { name: "View registered certificate" }),
    ).toBeInTheDocument();
    await user.click(within(dialog).getByRole("button", { name: "Create" }));
  ...
  it.each([
    ...
    "creates a user with %s authentication",
    async (_label, authValue, fieldLabel, payloadKey, fieldValue) => {
      enumUser.mockResolvedValue({ UserList: [alice] });
      createUser.mockResolvedValue({});
      const user = userEvent.setup();
  ...
  it("uploads a certificate and sends its bytes on save", async () => {
    ...
    const dialog = await screen.findByRole("dialog");
    expect(
      within(dialog).queryByRole("button", { name: "View registered certificate" }),
    ).not.toBeInTheDocument();

    const fileInput = dialog.querySelector('input[type="file"]') as HTMLInputElement;
    const file = new File([SELF_SIGNED_CERT_DER()], "user.cer", {
  ...
  it("extracts a user certificate from a password-protected PKCS #12 archive", async () => {
    ...
    dialog = await screen.findByRole("dialog");
    expect(within(dialog).getByLabelText("User certificate or PKCS #12 file name")).toHaveValue(
      "user.p12",
    );
  ...
  it("deletes a user after confirmation", async () => {
    ...
    expect(deleteUser.mock.calls[0][0]).toMatchObject({
      HubName_str: "DEFAULT",
      Name_str: "alice",
    });
