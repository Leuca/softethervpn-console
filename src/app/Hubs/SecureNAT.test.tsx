...
describe("SecureNAT", () => {
  ...
  it("loads runtime status and tables when Secure NAT is enabled", async () => {
    ...
    expect(
      await screen.findByRole("dialog", { name: "Secure NAT runtime information" }),
    ).toBeInTheDocument();
  ...
  it("normalizes the :: secondary DNS sentinel to an empty field", async () => {
    ...
    expect(
      screen.queryByText(/Secondary DNS server must be blank or a valid IPv4 address/i),
  ...
  it("disables actions on cluster servers", async () => {
    ...
    expect(
      await screen.findByText("Secure NAT is not available on cluster servers"),
    ).toBeInTheDocument();
