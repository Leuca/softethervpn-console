...
const findRoute = (label: string): IAppRoute | undefined =>
  flattenedRoutes.find((route) => route.label === label);
const findRouteByPath = (path: string): IAppRoute | undefined =>
  flattenedRoutes.find((route) => route.path === path);
...
describe("routes", () => {
  ...
  it("returns clear reasons for route denial states", () => {
    ...
    expect(
      routePermissionReason(localBridge as IAppRoute, {
        hideAdminOnly: false,
        hideNonCluster: false,
        hideNonBridge: false,
    ...
    expect(
      routePermissionReason(clusterConfig as IAppRoute, {
        hideAdminOnly: false,
        hideNonCluster: true,
        hideNonBridge: false,
        hiddenLabels: new Set<string>(),
    ...
    expect(
      routePermissionReason(layer3 as IAppRoute, {
        hideAdminOnly: false,
        hideNonCluster: false,
        hideNonBridge: true,
        hiddenLabels: new Set<string>(),
    ...
    expect(
      routePermissionReason(listeners as IAppRoute, {
        hideAdminOnly: false,
        hideNonCluster: false,
        hideNonBridge: false,
        hiddenLabels: new Set<string>(),
